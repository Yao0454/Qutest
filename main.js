...
function showResult() {
  ...
  // 圆环进度（周长 2π×78 ≈ 490.09）
  const pct = Math.min(finalScore / 125, 1);
  const ring = document.getElementById('ring-fill');
  ring.style.stroke = level.ring;
  setTimeout(() => { ring.style.strokeDashoffset = 490.09 * (1 - pct); }, 350);

  const dimInfo = [
    { cat: 'A', name: '身体荒废度', icon: '🛋️', color: '#ff6b35' },
    { cat: 'B', name: '数字沉溺度', icon: '📱', color: '#6b9fff' },
    { cat: 'C', name: '社会脱轨度', icon: '🏚️', color: '#00c494' },
    { cat: 'D', name: '社交萎缩度', icon: '🫥', color: '#ff7090' },
  ];

  const list = document.getElementById('dimension-grid');
  list.innerHTML = '';
  dimInfo.forEach(dim => {
    ...
    card.innerHTML = `
      ...
      <div class="dim-score-text">${dimScore} / ${maxPossible} 分 · ${pctDim}%</div>
    `;
